...
Ethereum discussions are centered around the possibilities opening up with the Turing-completeness of their script. We would like to present a different programming model. First of all, there is a cop out - you can write your own tradle code (today in Java, tomorrow in JavaScript, Python and other languages). But we consider this coding freedom only as a way of experimenting with the new use cases before they are understood to be programmed by non-developers. So what methods do we use to make micro-scripting feasible? If you look at Ethereum contracts, or any code for that matter, you will see a lot of scaffolding, with the key decisions, burried somewhere in all the fuss. We want those key decisions to be right at the surface. Below are some of the ways we are employing to achieve that:
...
- **chaining** - tradle firing can serve as an input into another tradle.
...
Who is Tradle
=============
Tradle is a self-funded US company, registered in May 2014 in Delaware, and operating in New Jersey, close to New York City. Tradle brings to the table its team's prior intelectual capital in the form of Web Database, with generic searching, browsing, filtering, data-entry, etc. and AWS cloud integration with automatic multi-level backup/restore, etc., with UI for data modeling and custom business rules written with JS scripts, and dozends of Web apps based on this technology (written in Java, not yet open sourced, but all sources are in GIT and have only open source dependencies), its [open source mobile web software](http://github.com/urbien/urbini) and its algo-trading marketplace Tradle, a mobile Web application that is built on top of the Web Database and Urbini, currently centralized and is being redesigned for decentralized operations based on the principles introduced in this paper.
