...
2. *on oracles* (utilizing extremely simple, fast and extremely scalable key-value data stores) as a form of a transaction log, coordinated by the on-chain data. Unlike public chain data, this type of data requires security (object level and property level), bandwidth, low latency, space, that regular bitcoin mining nodes and Ethereum nodes will likely not meet. Maidsafe's model of grading farmer's nodes is more suitable here. Note also that in Ethereum contract's storage is open and public, which is unsuitable for business data.
3. *on data nodes*, - a more typical database that is formed by syncing data from the chain and the oracles. This database is suitable for searching, browsing, filtering, etc. to enable business operations, and extended with  massive data archives, suitable for analytics. This storage is also used for syncing with the existing Web sites and legacy systems.
